"""CLI entry point for the Customer Discovery agent."""
...
@click.group()
def cli():
    """Ideation Agent: Customer Discovery - Interview framework."""
    pass
...
def run(session_id: str, problem: str | None, output_json: bool):
    ...
    console.print(f"[blue]Running customer-discovery agent for session: {session_id}[/blue]")
    result = run_agent(session_id, problem)
    memory.update_phase("customer_discovery", "complete", result)

    if output_json:
        print(json.dumps(result, indent=2))
    else:
        console.print("[green]Customer Discovery agent completed successfully[/green]")
